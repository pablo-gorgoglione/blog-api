...
exports.upVote = (req, res, next) => {
  ...
  if (req.params.idComment) {
    ...
    Like.findOne({ userId: idUser })
      ...
      .then((data) => {
        if (data) {
          return res.json(oResponse(0, "Only one like per user"));
        }
        newLike
          .save(newLike)
          .then((data) => {
            ...
            Comment.findById(idComment).then((comment) => {
              if (!comment) {
                return res.json(oResponse(0, "post does not exist"));
              }
              ...
              Comment.findByIdAndUpdate(idComment, comment, {
                ...
                if (!data) {
                  return res.json(
                    oResponse(0, "Cannot update the comment(like)")
                  );
                }
              });
              return res.json(oResponse(1, "Like created", data));
            });
          })
          .catch((err) => {
          ...
          .then((data) => {
            ...
            Post.findById(idPost).then((post) => {
              ...
              Post.findByIdAndUpdate(idPost, post, {
                ...
                if (!data) {
                  return res.json(oResponse(0, "Cannot update the post(like)"));
                }
              });
              return res.json(oResponse(1, "Like created", data));
            });
  ...
  if (req.params.idComment) {
    ...
    Like.findOne({ userId: idUser })
      ...
      .then((data) => {
        if (data) {
          Like.findByIdAndDelete(data._id)
            .then(() => {
              ...
              Comment.findById(idComment).then((comment) => {
                if (!comment) {
                  return res.json(oResponse(0, "comment does not exist"));
                }
                // comment.likeCounter = comment.likeCounter - 1;
                comment.likeCounter = comment.likeCounter - 1;
                Comment.findByIdAndUpdate(idComment, comment, {
        ...
        res.json(oResponse(0, "can't downvote if you didn't upvote"));
      });
  } else {
    //Like for the post
    ...
    Like.findOne({ userId: idUser })
      ...
      .then((data) => {
        if (data) {
            ...
            .then(() => {
              ...
              Post.findById(idPost).then((post) => {
                ...
                Post.findByIdAndUpdate(idPost, post, {
                  ...
                  if (!data) {
                    return res.json(
                      oResponse(0, "Cannot update the post(like)")
                    );
                  }
                });
              });
          ...
          return res.json(oResponse(1, "DisLike created", data));
        }
        return res.json(oResponse(0, "Can't downvote if you didn't upvote"));
      });
  }
};
